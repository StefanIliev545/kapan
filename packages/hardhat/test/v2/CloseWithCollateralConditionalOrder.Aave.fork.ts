/**
 * Fork Tests for Close With Collateral Conditional Orders - Aave V3
 *
 ...
 * 1. User has Aave position: collateral (wstETH) + debt (USDC)
 ...
 * - Uses AaveGatewayWrite instead of MorphoBlueGatewayWrite
 * - Protocol context is "0x" (empty) for Aave
 * - User needs to approve aToken for gateway (WithdrawCollateral)
 *
 * Run with: FORK_CHAIN=arbitrum npx hardhat test test/v2/CloseWithCollateralConditionalOrder.Aave.fork.ts
 */
...
// Protocol ID (bytes4)
const AAVE_PROTOCOL_ID = ethers.keccak256(ethers.toUtf8Bytes("aave-v3")).slice(0, 10) as `0x${string}`;
...
describe("Close With Collateral Conditional Order - Aave V3 (Fork)", function () {
  ...
  before(async function () {
    ...
    poolAddress = await poolProvider.getPool();
    ...
    // User approves aToken for gateway (required for WithdrawCollateral)
    ...
    const aToken = await ethers.getContractAt(erc20Abi, aWsteth);
    await aToken.connect(user).approve(await aaveGateway.getAddress(), ethers.MaxUint256);
  ...
  describe("Aave Close With Collateral: Sell wstETH for USDC", () => {
    it("should execute close position via flashLoanAndSettle with REAL frontend instructions", async function () {
      this.timeout(180000);

      /**
       * CLOSE WITH COLLATERAL FLOW FOR AAVE (exactly as frontend builds it):
       *
       ...
       * [0] Approve(input=1, aave) -> allows gateway to use USDC for repayment
       * [1] Repay(USDC, input=1, context=0x) -> repay user's debt
       * [2] WithdrawCollateral(wstETH, input=0, context=0x) -> withdraw collateral equal to actualSellAmount
      ...
      // This matches useClosePositionConfig.tsx lines 664-709 for Aave
      ...
      const appDataHash = ethers.keccak256(ethers.toUtf8Bytes("kapan-close-with-collateral-aave-test"));
      const salt = ethers.keccak256(ethers.toUtf8Bytes("close-pos-aave-" + Date.now()));
      ...
      // Set up Aave gateway in view router
      ...
      await viewRouter.setGateway("aave", await aaveGatewayView.getAddress());
      ...
      // wstETH at $3700, USDC at $1: 1000 USDC / 0.27 wstETH approx 3700 USDC/wstETH
      ...
      const [aTokenBalanceBefore] = await dataProvider.getUserReserveData(WSTETH, userAddress);
      const [, , usdcDebtBeforeActual] = await dataProvider.getUserReserveData(USDC, userAddress);

      console.log("\n=== Before Settlement ===");
      console.log(`Collateral (aToken balance): ${ethers.formatEther(aTokenBalanceBefore)} wstETH`);
      console.log(`USDC Debt: ${ethers.formatUnits(usdcDebtBeforeActual, 6)} USDC`);
      ...
      try {
        ...
        console.log("\n[OK] flashLoanAndSettle SUCCEEDED!");
      } catch (error: unknown) {
        console.log(`\n[FAIL] flashLoanAndSettle FAILED!`);
      ...
      // ============ VERIFY RESULTS ============
      const [aTokenBalanceAfter] = await dataProvider.getUserReserveData(WSTETH, userAddress);
      const [, , usdcDebtAfter] = await dataProvider.getUserReserveData(USDC, userAddress);

      console.log("\n=== After Settlement ===");
      console.log(`Collateral: ${ethers.formatEther(aTokenBalanceAfter)} wstETH (was ${ethers.formatEther(aTokenBalanceBefore)})`);
      console.log(`USDC Debt: ${ethers.formatUnits(usdcDebtAfter, 6)} (was ${ethers.formatUnits(usdcDebtBeforeActual, 6)})`);

      // Verify debt was repaid
      expect(usdcDebtAfter).to.be.lt(usdcDebtBeforeActual, "USDC debt should decrease");
      const debtReduction = usdcDebtBeforeActual - usdcDebtAfter;
      console.log(`\n[OK] Debt repaid: ${ethers.formatUnits(debtReduction, 6)} USDC`);
      expect(debtReduction).to.be.closeTo(triggerBuyAmount, triggerBuyAmount / 100n, "Debt reduction should match buy amount");

      // Verify collateral was withdrawn
      const collateralReduction = aTokenBalanceBefore - aTokenBalanceAfter;
      console.log(`[OK] Collateral withdrawn: ${ethers.formatEther(collateralReduction)} wstETH`);
      ...
      console.log(`[OK] Order status: Completed`);
      ...
      console.log(`[OK] Flash loan repaid (adapter wstETH: ${ethers.formatEther(adapterWstethBalance)})`);
      ...
      console.log(`[OK] No tokens stuck in OrderManager`);

      console.log("\n=== Close With Collateral Conditional Order Test (Aave V3) PASSED ===");
      console.log("Full flow executed:");
      console.log("  1. Flash loan: Borrowed wstETH from Morpho Blue");
      console.log("  2. Pre-hook: Moved wstETH to OrderManager");
      console.log("  3. Swap: Sold wstETH for USDC");
      console.log("  4. Post-hook: Repaid USDC debt to Aave, withdrew wstETH collateral");
