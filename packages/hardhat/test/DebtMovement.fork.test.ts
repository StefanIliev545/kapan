...
runOnlyOnFork("Debt Movement Integration Tests :fork", function () {
  ...
  it("Step 4: Should move debt back from Compound to Aave", async function () {
    ...
    const [fromCompoundApprovals, fromCompoundData] = await compoundGateway.getEncodedCollateralApprovals(
      WETH_ADDRESS,
      collaterals,
    );
    ...
    const [toAaveApprovals, toAaveData] = await aaveGateway.getEncodedDebtApproval(
      WETH_ADDRESS,
      compoundBorrowBalance,
      userAddress,
    );
    console.log("Got debt approval data from Aave Gateway");
    ...
    console.log(
      `Compound borrow balance after move back: ${ethers.formatUnits(compoundBorrowBalanceAfterMoveBack, 18)}`,
    );

    expect(compoundBorrowBalanceAfterMoveBack).to.be.lt(
      ethers.parseUnits("0.001", 18),
      "Compound debt should be nearly zero",
    );
