...
const deployKapanRouter: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  if (v2) {
    await safeExecute(hre, deployer, "KapanRouter", "setBalancerV2", [v2], { waitConfirmations: 5 });
  ...
  if (v3) {
    await safeExecute(hre, deployer, "KapanRouter", "setBalancerV3", [v3], { waitConfirmations: 5 });
  ...
  if (aaveEntry) {
    ...
    try {
      ...
      if (poolAddress && poolAddress !== ethers.ZeroAddress) {
        await safeExecute(hre, deployer, "KapanRouter", "setAavePool", [poolAddress], { waitConfirmations: 5 });
  ...
  if (morphoEntry) {
    ...
    try {
      await safeExecute(hre, deployer, "KapanRouter", "setMorphoBluePool", [morphoAddress], { waitConfirmations: 5 });
