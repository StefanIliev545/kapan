...
async function main() {
  console.log("=== Constructing Proper EIP-1271 Signature with Correct Hash ===\n");
  ...
  console.log("  sellTokenBalance:", tradeableOrder.sellTokenBalance);
  console.log("  buyTokenBalance:", tradeableOrder.buyTokenBalance);
  ...
  // Compute the order hash correctly
  // IMPORTANT: GPv2Order TYPE_HASH uses "string" for kind and balance types!
  const domainSeparator = await settlement.domainSeparator();
  console.log("Domain separator:", domainSeparator);

  // Correct TYPE_HASH - uses "string" not "bytes32" for kind and balances
  const ORDER_TYPE_HASH = ethers.keccak256(ethers.toUtf8Bytes(
    "Order(address sellToken,address buyToken,address receiver,uint256 sellAmount,uint256 buyAmount,uint32 validTo,bytes32 appData,uint256 feeAmount,string kind,bool partiallyFillable,string sellTokenBalance,string buyTokenBalance)"
  ));

  console.log("Order TYPE_HASH:", ORDER_TYPE_HASH);
  ...
  // Now submit to API using the order parameters
  // Map enums for API
  ...
  const apiOrder = {
    ...
    appData: tradeableOrder.appData, // Use the appData from tradeable order
    feeAmount: "0",
    kind: kindStr,
    partiallyFillable: tradeableOrder.partiallyFillable,
  ...
  try {
    ...
    if (response.ok) {
      ...
      // If signature error, show the hash the API computed
      const match = responseText.match(/0x[a-fA-F0-9]{64}/);
      if (match) {
        console.log("\nAPI computed hash:", match[0]);
        console.log("Our computed hash:", orderDigest);
        console.log("Match:", match[0] === orderDigest);
      }
    }
  } catch (e) {
