...
mod VesuGatewayV2 {
    ...
    impl InterestRateViewImpl of InterestRateView<ContractState> {
        ...
        fn get_borrow_rate(ref self: ContractState, token_address: ContractAddress) -> u256 {
            ...
            // V2: Get rate information directly from the pool
            let utilization = pool_dispatcher.utilization(token_address);
            let (asset_config, _) = pool_dispatcher.asset_config(token_address);
            
            // Note: In V2, interest rate calculation may need to be handled differently
            // This is a simplified implementation - may need adjustment based on actual V2 pool structure
            let year_in_seconds: u256 = 31536000; // 365 days
            let scale: u256 = pow(10,10);
            
            // Calculate basic interest rate (this may need adjustment for actual V2 implementation)
            let borrow_rate = (asset_config.fee_rate * year_in_seconds);
        ...
        fn get_supply_rate(ref self: ContractState, token_address: ContractAddress) -> u256 {
            ...
            // V2: Fetch rate data directly from pool
            let utilization = pool_dispatcher.utilization(token_address);
            let (asset_config, _) = pool_dispatcher.asset_config(token_address);
            ...
            // V2: Simplified calculation using fee_rate (may need adjustment for actual V2 implementation)
            // 1. Compute numerator: fee_rate * year * total_debt * last_rate 
            let mut numerator = asset_config.fee_rate * year_in_seconds;
            ...
            let supply_rate = if denominator > 0 { numerator / denominator } else { 0 };
