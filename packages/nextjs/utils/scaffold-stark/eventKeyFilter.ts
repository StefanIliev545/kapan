...
export const serializeEventKey = (input: any, abiEntry: AbiEntry, structs: AbiStructs, enums: AbiEnums): string[] => {
  ...
  for (const member of clonedKeyMembers) {
    if (member.value !== undefined) {
      if (!member.type.startsWith("core::array::Array::") && Array.isArray(member.value)) {
        keys = keys.concat(
          mergeArrays(
            member.value.map((matchingItem: any) =>
              serializeEventKey(matchingItem, member, structs, enums).map(item => [item]),
            ),
          ),
        );
        ...
        const serializedKeys = serializeEventKey(member.value, member, structs, enums).map(item => [item]);
