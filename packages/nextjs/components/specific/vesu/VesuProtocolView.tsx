    ...
    <div className="w-full flex flex-col p-4 space-y-6">
          ...
          <div className="space-y-4">
            {status === "connecting" || (userAddress && !hasLoadedOnce) ? (
              ...
              vesuRows.map(row => {
                ...
                const collateralSwitchOptions = assetsWithRates.filter(
                  asset => toHexAddress(asset.address) !== row.supply.tokenAddress,
                );
                const availableBorrowTokens = collateralSwitchOptions;
                ...
                const renderCollateralName = (defaultName: string) => {
                  if (collateralSwitchOptions.length === 0) {
                    return (
                      <>
                        <span className="font-semibold text-lg truncate">{defaultName}</span>
                  ...
                  const handleToggle = (event: MouseEvent<HTMLDivElement>) => {
                    event.stopPropagation();
                    setActiveSwitchMenu(isCollateralMenuOpen ? null : { key: row.key, type: "collateral" });
                  };

                  return (
                    <div className="dropdown dropdown-bottom" onClick={event => event.stopPropagation()}>
                      <div
                        tabIndex={0}
                        role="button"
                ...
                const renderDebtName = (defaultName: string) => {
                  if (!row.borrow || !row.debtAsset || debtSwitchOptions.length === 0) {
                    return <span className="font-semibold text-lg truncate">{defaultName}</span>;
                  }
                ...
                const closeButton = showCloseAction ? (
                  <button
                  ...
                  >
                    ...
                    >
                      <SupplyPosition
                        ...
                        position={positionManager}
                        disableMove
                        renderName={renderCollateralName}
      ...
      {switchSelection && switchSelection.type === "debt" && switchSelection.row.debtAsset && (
        <SwitchDebtModalStark
          isOpen={switchSelection !== null}
          onClose={() => setSwitchSelection(null)}
          poolId={poolId}
          collateral={{
            name: switchSelection.row.collateralSymbol,
            address: switchSelection.row.supply.tokenAddress,
            decimals: Number(switchSelection.row.collateralAsset.decimals),
            icon: tokenNameToLogo(switchSelection.row.collateralSymbol.toLowerCase()),
          }}
          currentDebt={{
            name:
              switchSelection.row.debtSymbol ?? feltToString(switchSelection.row.debtAsset.symbol),
            address:
              switchSelection.row.borrow?.tokenAddress ??
              toHexAddress(switchSelection.row.debtAsset.address),
            decimals: Number(switchSelection.row.debtAsset.decimals),
            icon: tokenNameToLogo(
              (switchSelection.row.debtSymbol ?? feltToString(switchSelection.row.debtAsset.symbol)).toLowerCase(),
            ),
          }}
          targetDebt={{
            name: feltToString(switchSelection.target.symbol),
            address: toHexAddress(switchSelection.target.address),
            decimals: Number(switchSelection.target.decimals),
            icon: tokenNameToLogo(feltToString(switchSelection.target.symbol).toLowerCase()),
          }}
          debtBalance={switchSelection.row.borrow?.tokenBalance ?? 0n}
          collateralBalance={switchSelection.row.supply.tokenBalance}
        />
      )}
      {switchSelection && switchSelection.type === "collateral" && (() => {
        const debtSymbolForSwitch =
          switchSelection.row.debtSymbol ??
          (switchSelection.row.debtAsset ? feltToString(switchSelection.row.debtAsset.symbol) : undefined);
        ...
        return (
          <SwitchCollateralModalStark
            ...
            debtToken={{
              name: debtSymbolForSwitch ?? "",
              address: switchSelection.row.borrow?.tokenAddress ?? "0x0",
              decimals: Number(switchSelection.row.debtAsset?.decimals ?? 18),
              icon: debtSymbolForSwitch ? tokenNameToLogo(debtSymbolForSwitch.toLowerCase()) : "",
            }}
            collateralBalance={switchSelection.row.supply.tokenBalance}
            debtBalance={switchSelection.row.borrow?.tokenBalance ?? 0n}
          />
      ...
      {closeSelection && closeSelection.borrow && closeSelection.debtAsset && (
        <ClosePositionModalStark
          isOpen={closeSelection !== null}
          onClose={() => setCloseSelection(null)}
          poolId={poolId}
          collateral={{
            name: closeSelection.collateralSymbol,
            address: closeSelection.supply.tokenAddress,
            decimals: Number(closeSelection.collateralAsset.decimals),
            icon: tokenNameToLogo(closeSelection.collateralSymbol.toLowerCase()),
          }}
          debt={{
            name: closeSelection.debtSymbol ?? feltToString(closeSelection.debtAsset.symbol),
            address: closeSelection.borrow.tokenAddress,
            decimals: Number(closeSelection.debtAsset.decimals),
            icon: tokenNameToLogo(
              (closeSelection.debtSymbol ?? feltToString(closeSelection.debtAsset.symbol)).toLowerCase(),
            ),
          }}
          collateralBalance={closeSelection.supply.tokenBalance}
          debtBalance={closeSelection.borrow.tokenBalance}
        />
      )}
    </div>
  );
