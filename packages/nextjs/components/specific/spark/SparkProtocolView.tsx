...
import { useSparkEMode } from "~~/hooks/useAaveEMode";
...
export const SparkProtocolView: FC<{ chainId?: number; enabledFeatures?: { swap?: boolean; move?: boolean } }> = ({ chainId, enabledFeatures }) => {
  ...
  // Type assertion needed because SparkGatewayView may not be in ContractName yet
  const { data: gateway } = useScaffoldContract({ contractName: "SparkGatewayView" as any, chainId: chainId as any });
  ...
  const { userEMode, userEModeId } = useSparkEMode(chainId);
  ...
  return (
    <AaveLike chainId={chainId} contractName="SparkGatewayView" key={refreshKey}>
